...
if uploaded_file:
  ...
  # --- KRİTİK GÜNCELLEME: Tarih Hatalarını Gideren Kısım ---
  ...
  # Nöbet başlangıcı boş olan (NaT) satırları temizleyelim ki analiz bozulmasın
  ...
  with tab_ana:
      ...
      if f_df.empty:
          ...
          total_o_master_sum = int(master_plan['Onerilen_Güvenli_Kapasite'].sum())
          avg_p = total_p_sum / num_days
          avg_k = total_k_sum / num_days
          avg_o = float(total_o_master_sum)
          avg_s = avg_p - avg_o
          ...
          num_c = ['Mevcut_Planlanan', 'Fiili_Kullanilan', 'Onerilen_Güvenli_Kapasite', 'Fark_Mevcut_Onerilen']
          t_row = summary_df[num_c].sum().to_frame().T
          t_row['Tarih'] = 'TOTAL'
          a_row = summary_df[num_c].mean().to_frame().T
          a_row['Tarih'] = 'AVERAGE'
          styled_daily = pd.concat([summary_df, t_row, a_row], ignore_index=True)
          st.dataframe(styled_daily.style.apply(style_risk, axis=1).format({c: "{:g}" for c in num_c}), use_container_width=True, hide_index=True)
          st.info(f"💡 **Operasyonel Transfer Analizi:** {lead_time} saat lead time kuralına göre yapılan atamalar sonucunda risk oranı **%{risk_orani_oncesi:.1f}**'den **%{risk_orani_sonrasi:.1f}** seviyesine indirilmiştir.")
          st.divider()
          ...
          mp_sum_vals = master_plan[mp_cols[1:]].sum().to_frame().T
          mp_sum_vals['Saat'] = 'TOTAL'
          mp_final = pd.concat([master_plan[mp_cols], mp_sum_vals], ignore_index=True)
          st.dataframe(mp_final.style.apply(lambda x: ['font-weight: bold; background-color: #f0f2f6' if x['Saat'] == 'TOTAL' else '' for _ in x], axis=1).format({c: "{:g}" for c in mp_cols[1:]}), use_container_width=True, hide_index=True)
  with tab_planlamaci:
      ...
      if not f_df.empty:
          ...
          p2.markdown(f'<div class="kpi-card"><div class="kpi-title">Net Tasarruf (Dönem Toplam)</div><div class="kpi-value">{int(avg_s * num_days)}</div></div>', unsafe_allow_html=True)
          ...
          # --- GÜNCELLENEN KISIM ---
          h_plan = master_plan[['Saat', 'Onerilen_Güvenli_Kapasite']].sort_values('Saat').copy()
          h_plan['Ort. Planlanan Nöbetçi'] = h_plan['Saat'].map(daily_detail.groupby('Saat')['Mevcut_Planlanan'].mean()).apply(lambda x: int(np.floor(x + 0.5)))
          h_plan['Ort. Kullanılan Nöbetçi'] = h_plan['Saat'].map(daily_detail.groupby('Saat')['Fiili_Kullanilan'].mean()).apply(lambda x: int(np.floor(x + 0.5)))
          h_cols = ['Saat', 'Ort. Planlanan Nöbetçi', 'Ort. Kullanılan Nöbetçi', 'Onerilen_Güvenli_Kapasite']
          h_total = pd.DataFrame({
              'Saat': ['TOTAL'],
              'Ort. Planlanan Nöbetçi': [int(h_plan['Ort. Planlanan Nöbetçi'].sum())],
              'Ort. Kullanılan Nöbetçi': [int(h_plan['Ort. Kullanılan Nöbetçi'].sum())],
              'Onerilen_Güvenli_Kapasite': [int(h_plan['Onerilen_Güvenli_Kapasite'].sum())]
          })
          st.dataframe(pd.concat([h_plan[h_cols], h_total], ignore_index=True).style.apply(
              lambda x: ['font-weight: bold; background-color: #f0f2f6' if x['Saat'] == 'TOTAL' else '' for _ in x], axis=1
          ).format({c: "{:g}" for c in h_cols[1:]}), use_container_width=True, hide_index=True)
          # --- GÜNCELLEME SONU ---
  with tab_strateji:
                  ...
                  for _, row in combos.iterrows():
                      ...
                      for prof in test_profiles:
                          ...
                          global_exec_summary.append({
                              ...
                              'Mevcut Plan (Ort)': round(c_avg_p, 1), 'Önerilen Nöbetçi Sayısı': int(c_avg_o),
      ...
      if st.session_state.strateji_sonuc is not None:
          ...
          st.dataframe(filtered_df.style.apply(lambda x: ['background-color: #d8f3dc; font-weight: bold; color: black'] * len(x) if x['Optimum'] else [''] * len(x), axis=1).format({
              'Mevcut Plan (Ort)': "{:.1f}", 'Önerilen Nöbetçi Sayısı': "{:g}", 'Net Tasarruf': "{:.1f}", 'Risk Adedi (Saat)': "{:g}",
              'Op. Risk Oranı (%)': "{:.1f}", 'Yön. Risk Endeksi (%)': "{:.2f}"
