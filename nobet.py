...
if uploaded_file:
    ...
    with tab_ana:
        ...
        if f_df.empty:
            ...
            total_o_sum, total_fark_sum = daily_detail['Onerilen_Güvenli_Kapasite'].sum(), daily_detail['Fark'].sum()
    ...
    with tab_planlamaci:
        ...
        if f_df.empty:
            ...
            p2.markdown(f'<div class="kpi-card"><div class="kpi-title">Net Tasarruf (Aylık Toplam)</div><div class="kpi-value">{total_fark_sum:.0f}</div></div>', unsafe_allow_html=True)
            ...
            # 1. ASIL TABLO: VARDİYA BAZLI GRUPLAMA + GRAND TOTAL
            ...
            vardiya_ozet = plan_master.groupby('Saat Aralığı').agg(
                Toplam_Onerilen_Adet=('Onerilen_Güvenli_Kapasite', 'sum')
            ).reset_index()
            ...
            # 2. ALT DETAY: SAATLİK LİSTE + GRAND TOTAL (AÇILIR TABLO YAPILDI)
    ...
    with tab_strateji:
                    ...
                    for _, row in combos.iterrows():
                        ...
                        for prof in test_profiles:
                            ...
                            global_exec_summary.append({
                                ...
                                'Mevcut Plan (Ort)': round(c_avg_p, 1), 'Önerilen Plan (Ort)': round(c_avg_o, 1),
        ...
        # Veri hafızadaysa filtreleri ve tabloyu göster (Butonun DIŞINDA)
        if st.session_state.strateji_sonuc is not None:
            ...
            with f1:
                filter_seviye = st.multiselect("Analiz Seviyesi", sorted(g_df['Analiz Seviyesi'].unique()))
            with f2:
            ...
            if filter_seviye:
                filtered_df = filtered_df[filtered_df['Analiz Seviyesi'].isin(filter_seviye)]
            if filter_base:
            ...
            # --- YEŞİL RENKLENDİRME VE GÖRÜNÜM ---
            def style_optimum(row):
                if row['Optimum']:
                    return ['background-color: #d8f3dc; font-weight: bold; color: black'] * len(row)
                return [''] * len(row)

            st.dataframe(
                filtered_df.style.apply(style_optimum, axis=1).format(precision=1), 
