...
if uploaded_file:
   ...
   with tab_ana:
       ...
       if f_df.empty:
           ...
           daily_2hr_totals = daily_hourly.groupby(['Tarih', '2_Saat_Araligi']).agg({'Mevcut_Planlanan': 'sum', 'Fiili_Kullanilan': 'sum'}).reset_index()
           two_hour_analysis = daily_2hr_totals.groupby('2_Saat_Araligi').agg(Mevcut_Planlanan_Ort=('Mevcut_Planlanan', 'mean'), Fiili_Kullanilan_Ort=('Fiili_Kullanilan', 'mean'), Onerilen_Güvenli_Kapasite=('Fiili_Kullanilan', lambda x: np.ceil(np.percentile(x, risk_profile)).astype(int))).reset_index()
               ...
               for label, col in levels:
                   ...
                   for _, row in combos.iterrows():
                       ...
                       for prof in test_profiles:
                           ...
                           # RİSK ADEDİ HESAPLAMA (Saatlik bazda öneriyi aşan durum sayısı)
                           ...
                           global_exec_summary.append({
                               ...
                               'Risk Adedi (Saat)': int(c_r_count), # Yeni Kolon Eklendi
