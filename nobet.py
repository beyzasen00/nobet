...
if uploaded_file:
  ...
  with tab_strateji:
              ...
              for label, col in levels:
                  ...
                  for _, row in combos.iterrows():
                      ...
                      for prof in test_profiles:
                          ...
                          # Yönetici Risk Endeksi (Yeni Tanım)
      ...
      if st.session_state.strateji_sonuc is not None:
          ...
          st.markdown("""<div class="highlight-box"><b>💡 Öneri Nasıl Hesaplanıyor?</b><br>1. Veriler saatlik bazda gruplanır ve seçilen <b>Güven Aralığına</b> göre istatistiksel üst sınır belirlenir.<br>2. <b>Yeşil satırlar;</b> Risk oranı %5'ten küçük en yüksek tasarrufu; yoksa tasarruf sağlayan ilk en düşük riskli noktayı temsil eder.<br><b>Yön. Risk Endeksi (%):</b> Riskli saatlerdeki toplam açığın, o senaryodaki genel fiili kullanıma oranıdır.<br><b>Yaz1:</b> Haz-Eyl | <b>Yaz2:</b> Nis-May-Eki | <b>Kış:</b> Kas-Mar</div>""", unsafe_allow_html=True)
          output_g = BytesIO()
