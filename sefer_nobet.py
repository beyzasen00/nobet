...
if uploaded_file:
   ...
   sinif = st.sidebar.multiselect("Sınıf", options=df['Uçucu Sınıfı'].unique(), default=df['Uçucu Sınıfı'].unique())
   mask = (df['Ay_Key'].isin(selected_ay)) & (df['Baz Filo'].isin(filo)) & (df['Uçucu Sınıfı'].isin(sinif))
   f_all = df[mask].copy()
   ...
   def calculate_need_time(row):
       ...
       offset = 4 if any(p in str(row['Uçucu Sınıfı']) for p in ["Kaptan", "First Officer"]) or "Ev" in str(row['Nöbet Türü']) else 2
   ...
   if not f_giden.empty:
       ...
       # --- KRİTİK DÜZELTME: 6 SAAT SINIRLI TRANSFER ---
       ...
       for i in range(len(final_table)):
           ...
           if current_net < 0:
               ...
               # Sadece son 6 SATIRI kontrol et (Her satır 1 saattir)
               # i=10 ise range(4, 10) olur -> 4,5,6,7,8,9. indekslere bakar.
               lookback_range = range(max(0, i-6), i)
               logs = []
               for prev_idx in reversed(list(lookback_range)):
                   # Zaman farkını da garantiye alalım (Data eksikliği varsa aradaki saatleri atlamasın)
       ...
       with rkpi1: st.markdown(f'<div class="kpi-card kpi-orange"><div class="kpi-label">Öneri Üstü (Riskli) Saatler</div><div class="kpi-val">{total_risks}</div></div>', unsafe_allow_html=True)
       with rkpi2: st.markdown(f'<div class="kpi-card kpi-green"><div class="kpi-label">Transfer ile Çözülen</div><div class="kpi-val">{resolved}</div></div>', unsafe_allow_html=True)
       with rkpi3: st.markdown(f'<div class="kpi-card kpi-red"><div class="kpi-label">Kritik Risk (Çözülemeyen)</div><div class="kpi-val">{critical_risks}</div></div>', unsafe_allow_html=True)
   else:
